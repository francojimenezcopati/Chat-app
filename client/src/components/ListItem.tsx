...
const ListItem: React.FC<Props> = ({ chatMembership }) => {
	...
	const onExpelClick = () => {
		console.log("Kick from chat");
	};

	return (
		<div
			className={
				"border-y border-slate-600 w-full p-2 text-lg flex justify-between items-center -mb-[1px]"
			}
		>
				...
				{!chatMembership.isAdmin && (
					<Tooltip>
						<ConfirmModal onConfirm={onGiveAdminClick}>
							<TooltipTrigger className="flex justify-center items-center">
								<img src="key.svg" className="hover:cursor-pointer w-5 h-5" />
							</TooltipTrigger>
						</ConfirmModal>
						<TooltipContent>
							<p>Give admin</p>
